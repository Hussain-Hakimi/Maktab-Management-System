...
public class AttendanceSummaryTests
{
    private sealed class InMemoryAttendanceRepository : IAttendanceRepository
    {
        ...
        public Task<IReadOnlyList<AttendanceRecord>> GetByClassAndDateAsync(int classId, DateTime date, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<AttendanceRecord>>(Records.Where(r => r.Date.Date == date.Date).ToList());
        public Task<IReadOnlyList<AttendanceRecord>> GetByStudentAndRangeAsync(int studentId, DateTime startDate, DateTime endDate, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<AttendanceRecord>>(Records.Where(r => r.StudentId == studentId && r.Date.Date >= startDate.Date && r.Date.Date <= endDate.Date).ToList());
        ...
        public Task<int> GetAbsenceDaysByStudentAndRangeAsync(int studentId, DateTime startDate, DateTime endDate, CancellationToken cancellationToken = default) => Task.FromResult(Records.Count(r => r.StudentId == studentId && r.Status == AttendanceStatus.Absent && r.Date.Date >= startDate.Date && r.Date.Date <= endDate.Date));
    ...
    public async Task GetStudentAttendanceSummary_CalculatesCountsCorrectly()
    {
        ...
        studentRepo.Students.Add(new Student { StudentId = 1, FirstName = "Ahmad", LastName = "Karimi", FatherName = "Mohammad", ClassId = 1, RollNumber = "101" });
