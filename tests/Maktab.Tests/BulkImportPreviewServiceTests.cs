...
public sealed class BulkImportPreviewServiceTests
{
    ...
    public async Task PreviewStudents_WithInvalidRow_ReturnsErrorsWithoutWriting()
    {
        ...
        var classes = new ClassSubjectService();
        ...
        Assert.Empty(students.Students);
    }
    ...
    {
        var students = new StudentService();
        students.Students.Add(new Student { StudentId = 1, ClassId = 1, RollNumber = "101" });
        var classes = new ClassSubjectService();
        classes.Subjects.Add(new Subject { SubjectId = 10, ClassId = 1, SubjectName = "Math" });
        var service = new BulkImportPreviewService(students, classes, new ExcelReader());

        var csv = "RollNumber,MidtermScore,FinalScore\n101,18,50";
        var result = await service.PreviewMarksFromCsvAsync(csv, 1, 10, 1);
    ...
    public async Task PreviewAttendance_WithInvalidStatus_RejectsRow()
    {
        ...
        var result = await service.PreviewAttendanceFromCsvAsync(csv, 1, 1);
