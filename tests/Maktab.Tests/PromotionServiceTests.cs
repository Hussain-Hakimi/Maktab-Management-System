...
public class PromotionServiceTests
{
    private sealed class InMemoryStudentRepository : IStudentRepository
    {
        ...
        public Task<IReadOnlyList<Student>> GetStudentsByClassAndAcademicYearAsync(int classId, int academicYearId, CancellationToken cancellationToken = default) => throw new NotImplementedException();
        public Task<IReadOnlyList<StudentAcademicEnrollment>> GetStudentAcademicHistoryAsync(int studentId, CancellationToken cancellationToken = default) => throw new NotImplementedException();
    ...
    private sealed class InMemoryExamMarkRepository : IExamMarkRepository
    {
        ...
        public Task<IReadOnlyList<ExamMark>> GetMarksByClassAndSubjectAsync(int classId, int subjectId, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<ExamMark>>(Marks.Where(m => m.SubjectId == subjectId).ToList());
        public Task<IReadOnlyList<ExamMark>> GetMarksByClassSubjectAndYearAsync(int classId, int subjectId, int academicYearId, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<ExamMark>>(Marks.Where(m => m.SubjectId == subjectId && m.AcademicYearId == academicYearId).ToList());
        public Task<IReadOnlyList<ExamMark>> GetMarksByStudentAsync(int studentId, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<ExamMark>>(Marks.Where(m => m.StudentId == studentId).ToList());
    ...
    private sealed class InMemoryAttendanceRepository : IAttendanceRepository
    {
        ...
        public Task<int> GetAbsenceDaysByStudentAndYearAsync(int studentId, int academicYearId, CancellationToken cancellationToken = default) => Task.FromResult(Records.Count(r => r.StudentId == studentId && r.AcademicYearId == academicYearId && r.Status == AttendanceStatus.Absent));
        public Task<IReadOnlyList<AttendanceRecord>> GetByStudentAndYearAsync(int studentId, int academicYearId, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<AttendanceRecord>>(Records.Where(r => r.StudentId == studentId && r.AcademicYearId == academicYearId).ToList());
        public Task<IReadOnlyList<AttendanceRecord>> GetByClassAndYearAsync(int classId, int academicYearId, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<AttendanceRecord>>(Records.Where(r => r.AcademicYearId == academicYearId).ToList());
    }
    ...
    private sealed class InMemoryPromotionHistoryRepository : IStudentPromotionHistoryRepository
    {
        ...
        public Task<int> AddAsync(StudentPromotionHistory history, CancellationToken cancellationToken = default) { history.PromotionId = Histories.Count + 1; Histories.Add(history); return Task.FromResult(history.PromotionId); }
        public Task<IReadOnlyList<StudentPromotionHistory>> GetByStudentAsync(int studentId, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<StudentPromotionHistory>>(Histories.Where(h => h.StudentId == studentId).ToList());
        public Task<IReadOnlyList<PromotionHistoryDto>> GetHistoryAsync(int? academicYearId, int? studentId, CancellationToken cancellationToken = default)
        {
            ...
            var result = query.Select(h => new PromotionHistoryDto { PromotionId = h.PromotionId, StudentId = h.StudentId, StudentName = h.StudentId.ToString(), RollNumber = h.StudentId.ToString(), FromClassName = h.FromClassId.ToString(), ToClassName = h.ToClassId?.ToString(), AcademicYearName = h.AcademicYearId.ToString(), Result = h.Result, PromotionDate = h.PromotionDate }).ToList();
    ...
    public async Task RunPromotion_WhenStudentPassesAll_UpdatesClassAndRecordsHistory()
    {
        ...
        studentRepo.Students.Add(new Student { StudentId = 1, FirstName = "Ahmad", LastName = "Karimi", FatherName = "Mohammad", ClassId = 1, RollNumber = "101", RegistrationDate = DateTime.Now });
    ...
    public async Task GetPromotionHistory_ReturnsCorrectRecords()
    {
        ...
        var attendanceRepo = new InMemoryAttendanceRepository();
        var historyRepo = new InMemoryPromotionHistoryRepository();
        historyRepo.Histories.Add(new StudentPromotionHistory { StudentId = 1, FromClassId = 1, ToClassId = 2, AcademicYearId = 1, Result = "Promoted", PromotionDate = DateTime.Now });
        historyRepo.Histories.Add(new StudentPromotionHistory { StudentId = 2, FromClassId = 1, ToClassId = null, AcademicYearId = 1, Result = "Repeat", PromotionDate = DateTime.Now });
        var service = new PromotionService(studentRepo, classRepo, markRepo, attendanceRepo, historyRepo);
