...
public partial class BulkImportView
{
    ...
    {
        if (sender is not Button button || button.Name is not ("ImportStudentsButton" or "ImportMarksButton" or "ImportAttendanceButton" or "ImportMultiMarksButton")) return;
    ...
    private async Task PreviewAndImportAsync(string buttonName)
    {
        ...
        catch (Exception ex) { MessageBox.Show($"خطا در بررسی فایل:\n{ex.Message}", "خطا", MessageBoxButton.OK, MessageBoxImage.Error); }
    }

    private async Task PreviewStudentsAndImportAsync()
    {
        var preview = !string.IsNullOrWhiteSpace(_studentsFilePath) ? await BulkImportPreviewService.PreviewStudentsFromFileAsync(_studentsFilePath) : await BulkImportPreviewService.PreviewStudentsFromCsvAsync(StudentsCsvTextBox.Text.Trim());
        ...
        var result = !string.IsNullOrWhiteSpace(_studentsFilePath) ? await _bulkImportService.ImportStudentsFromFileAsync(_studentsFilePath) : await _bulkImportService.ImportStudentsFromCsvAsync(StudentsCsvTextBox.Text.Trim());
    ...
    {
        if (MarksClassComboBox.SelectedValue is not int classId || classId <= 0 || MarksSubjectComboBox.SelectedValue is not int subjectId || subjectId <= 0 || MarksYearComboBox.SelectedValue is not int yearId || yearId <= 0) { MessageBox.Show("لطفاً صنف، مضمون و سال تعلیمی را انتخاب کنید.", "اطلاعات ناقص", MessageBoxButton.OK, MessageBoxImage.Warning); return; }
        var preview = !string.IsNullOrWhiteSpace(_marksFilePath) ? await BulkImportPreviewService.PreviewMarksFromFileAsync(_marksFilePath, classId, subjectId, yearId) : await BulkImportPreviewService.PreviewMarksFromCsvAsync(MarksCsvTextBox.Text.Trim(), classId, subjectId, yearId);
        ...
        var result = !string.IsNullOrWhiteSpace(_marksFilePath) ? await _bulkImportService.ImportMarksFromFileAsync(_marksFilePath, classId, subjectId, yearId) : await _bulkImportService.ImportMarksFromCsvAsync(MarksCsvTextBox.Text.Trim(), classId, subjectId, yearId);
    ...
    {
        if (AttendanceClassComboBox.SelectedValue is not int classId || classId <= 0 || AttendanceYearComboBox.SelectedValue is not int yearId || yearId <= 0) { MessageBox.Show("لطفاً صنف و سال تعلیمی را انتخاب کنید.", "اطلاعات ناقص", MessageBoxButton.OK, MessageBoxImage.Warning); return; }
        var preview = !string.IsNullOrWhiteSpace(_attendanceFilePath) ? await BulkImportPreviewService.PreviewAttendanceFromFileAsync(_attendanceFilePath, classId, yearId) : await BulkImportPreviewService.PreviewAttendanceFromCsvAsync(AttendanceCsvTextBox.Text.Trim(), classId, yearId);
        ...
        var result = !string.IsNullOrWhiteSpace(_attendanceFilePath) ? await _bulkImportService.ImportAttendanceFromFileAsync(_attendanceFilePath, classId, yearId) : await _bulkImportService.ImportAttendanceFromCsvAsync(AttendanceCsvTextBox.Text.Trim(), classId, yearId);
    ...
    {
        if (MultiMarksClassComboBox.SelectedValue is not int classId || classId <= 0 || MultiMarksYearComboBox.SelectedValue is not int yearId || yearId <= 0 || string.IsNullOrWhiteSpace(_multiMarksFilePath)) { MessageBox.Show("لطفاً صنف، سال تعلیمی و فایل Excel را انتخاب کنید.", "اطلاعات ناقص", MessageBoxButton.OK, MessageBoxImage.Warning); return; }
    ...
    private static bool ConfirmImport(BulkImportPreviewResult preview, string itemName) => MessageBox.Show($"بررسی فایل تکمیل شد.\n\nتعداد ردیف‌های معتبر: {preview.ValidRows}\nتعداد ردیف‌ها: {preview.TotalRows}\n\nآیا می‌خواهید {itemName} را وارد سیستم کنید؟\nاین عملیات اطلاعات فایل را در دیتابیس ذخیره خواهد کرد.", "تأیید ورود", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes;
    private static void ShowPreviewResult(BulkImportPreviewResult result, TextBlock summary, TextBox errors, string name) { summary.Text = $"پیش‌نمایش {name}: {result.ValidRows} ردیف معتبر از {result.TotalRows} ردیف"; errors.Text = result.Errors.Count == 0 ? "پیش‌نمایش موفق بود؛ خطایی یافت نشد." : string.Join(Environment.NewLine, result.Errors); }
    private static void OfferErrorReport(IReadOnlyList<string> errors, string defaultName) { if (errors.Count == 0) return; var dialog = new SaveFileDialog { FileName = defaultName, DefaultExt = ".txt", Filter = "Text Files (*.txt)|*.txt" }; if (dialog.ShowDialog() != true) return; File.WriteAllLines(dialog.FileName, errors); MessageBox.Show($"گزارش خطا ذخیره شد:\n{dialog.FileName}", "گزارش خطا", MessageBoxButton.OK, MessageBoxImage.Information); }
